...
export async function prioritizeItem(itemId: string, workspaceSlug: string) {
  const supabase = getSupabaseServerClient();
  if (!supabase) {
    return { success: false, error: "Supabase not configured" };
  }
...
export async function archiveItem(itemId: string, workspaceSlug: string) {
  const supabase = getSupabaseServerClient();
  if (!supabase) {
    return { success: false, error: "Supabase not configured" };
  }

  // Archive by marking as not actionable, not unread
  const { error } = await supabase
    .from("assistant_items")
    .update({ is_actionable: false, is_unread: false })
    .eq("id", itemId);
